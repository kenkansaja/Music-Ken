              ...
              try:
                ...
                emojilist = [
                  ...
                  while j < 10:
                    ...
                    keyboard = InlineKeyboardMarkup(
                           [
                            [InlineKeyboardButton("1", callback_data=f"plll 0|{query}|{user_id}"), InlineKeyboardButton("2", callback_data=f"plll 1|{query}|{user_id}"), InlineKeyboardButton("3", callback_data=f"plll 2|{query}|{user_id}")],
                            [InlineKeyboardButton("4", callback_data=f"plll 3|{query}|{user_id}"), InlineKeyboardButton("5", callback_data=f"plll 4|{query}|{user_id}"), InlineKeyboardButton("6", callback_data=f"plll 5|{query}|{user_id}")],
                            [InlineKeyboardButton(text="🗑 ʙᴀᴛᴀʟ 🗑️", callback_data="cls")],
                           ]
                      )
                    await message.reply_photo(
                                    photo=KENKAN, caption=toxxt, reply_markup=keyboard
                                )
                                await lel.delete()
                    return
                                # 𝗚𝗢𝗛𝗔𝗡 𝗠𝗨𝗦𝗜𝗖 tolol
                            except:
                                await lel.edit("**❌ Error ")
