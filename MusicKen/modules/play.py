...
async def generate_cover(requested_by, title, views, duration, thumbnail):
    ...
    draw.text((205, 550), f"Title: {title}", (0, 0, 0), font=font)
    draw.text((205, 590), f"Duration: {duration}", (0, 0, 205), font=font)
    draw.text((205, 630), f"Views: {views}", (0, 0, 205), font=font)
    draw.text((205, 670), f"Added By: {requested_by}", (0, 0, 205), font=font,
    )
    img.save("final.png")
    os.remove("temp.png")
...
async def play(_, message: Message):
    ...
    if message.chat.id in DISABLED_GROUPS:
        return
    lel = await message.reply("🔄 <b>Memproses...</b>")
            ...
            if administrator == message.from_user.id:
                ...
                try:
                    ...
                    await USER.send_message(
                        message.chat.id, "Saya bergabung untuk memainkan musik pada obrolan suara/vcg!"
                    )
                    await lel.edit(
    ...
    for i in message.command[1:]:
        query += " " + str(i)
    print(query)
    await lel.edit("🎶 <b>Memproses lagu yang diminta</b>")
    ...
    try:
        ...
        await lel.edit(
            "<i>Lagu tidak ditemukan. </i> Coba cari dengan judul lagu yang lebih jelas, Ketik `/help` bila butuh bantuan"
        )
        print(str(e))
        return
    try:    
